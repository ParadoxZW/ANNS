...
typedef std::vector<float *> MMBase;
...
struct Candidate { // TODO: class
    size_t idx;
    float dist;
    Candidate(size_t idx, VectorXf &query, MMBase &base) {
        this->idx = idx;
        // this->dist = (query - base.row(idx).adjoint()).array().square().sum();
        float *target = base[idx];
        float d = 0;
...
void mmap_load(const char *filename, std::vector<float *> &data, unsigned &num,
               unsigned &dim) {
...
void graph_search_(MMBase &database, VectorXf &query, Graph &graph, size_t start_idx,
...
double graph_search(MMBase &database, MatrixXf &querytable, Graph &graph,
...
int main(int argc, char **argv) {
    ...
    mmap_load(argv[3], database, points_num, dim);
