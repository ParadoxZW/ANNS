    ...
    public:
        ...
        float Dist(VectorXf &query, unsigned idx) {
            ...
            for (unsigned i = 0; i < query.size(); i++) {
                t = ct[i / sub][(unsigned)(code[idx][i / sub] - 1)][i % sub];
                d += (rq[i] - t) * (rq[i] - t);
            }
            return d;
        }
...
int main(int argc, char **argv) {
    ...
    unsigned tmp1, tmp2;
    char cfile[20] = "./opq-data/c0.fvecs";
    ...
    load_fvecs("./opq-data/R.fvecs", index.R, tmp1, tmp2);
    std::cout << tmp1 << " " << tmp2 << std::endl;
    load_cvecs("./opq-data/code.cvecs", index.code, points_num, index.M);
