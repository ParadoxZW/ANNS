function [centers_table, idx_table, distortion, R] = train_opq_np(X, M,centers_table, R, num_iter_outer, num_iter_inner)
...
for iter_outer = 1:num_iter_outer
    ...
    Xproj = single(X)*single(R); % pre-projecting X
    ...
    for m = 1:M
        ...
        [idx, centers] = kmeans(Xsub, k, 'Options', opts, 'Start', single(centers_table{m}), 'EmptyAction', 'singleton');
        centers_table{m} = centers;
        
        % line 6 in Algorithm 1
        dist = sqdist(single(centers'), Xsub');
        [dist, idx] = min(dist);
        idx_table(:,m) = idx(:);
