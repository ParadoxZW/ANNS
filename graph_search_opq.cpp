...
int main(int argc, char **argv) {
    ...
    std::string s1 = "./opq";
    std::string s1_ = "/c";
    s1 = s1 + std::to_string(index.M) + s1_;
    std::string s2 = ".fvecs";
    for (unsigned i = 0; i < index.M; i++) {
        // cfile[7] = i + '0';
        MatrixXf cb;
        index.ct.push_back(cb);
        load_fvecs((s1+std::to_string(i+1)+s2).c_str(), index.ct[i], tmp1, tmp2);
        // std::cout << tmp1 << " " << tmp2 << std::endl;
    }

    load_fvecs("./opq16/R.fvecs", index.R, tmp1, tmp2);
    // std::cout << index.R[0][0] << " " << index.R[0][1] << std::endl;
    // std::cout << index.R[1][0] << " " << index.R[1][1] << std::endl;
    load_cvecs("./opq16/code.cvecs", index.code, points_num, index.M);
