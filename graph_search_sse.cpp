...
float SSEDist(const float *v1, const float *v2, unsigned size) {
    int nBlockWidth = 4;
    int cntBlock = size / nBlockWidth;
    int cntRem = size % nBlockWidth;

    float output = 0;
    __m128 loadData1, loadData2, loadData3;
    ...
    for (int i = 0; i < cntBlock; i++) {
        ...
        loadData3 = _mm_sub_ps(loadData1, loadData2);
        sumData = _mm_add_ps(sumData, _mm_mul_ps(loadData3, loadData3));
        p1 += nBlockWidth;
        p2 += nBlockWidth;
    }
    sumData = _mm_hadd_ps(sumData, sumData); // p[0] + p[1] + p[4] + p[5] + ...
    sumData = _mm_hadd_ps(sumData, sumData); // p[2] + p[3] + p[6] + p[7] + ...
    output += _mm_cvtss_f32(sumData);
...
struct Candidate { // TODO: class
    ...
    Candidate(size_t idx, VectorXf &query, MatrixXf &base) {
        ...
        // }
        // this->dist = d;
        this->dist = SSEDist(query.data(), target.data(), query.size());
    }
